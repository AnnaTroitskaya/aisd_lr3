...
void fill_unique(int arr[], int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        do {
            isUnique = true;
            randomNumber = rand() % 3000; // Генерация случайного числа от 1 до 100
...
void menu() 
{
	...
	{
		short int cmd;
		cout << "\nSelect an action:\n\n0.Exit the programm\n1.Compare trees' heights\n2.Compare balancing time\n3.Trees' traversals\n";
		cin >> cmd;
		...
		case(1):
        {
            ...
            for (int size = 200; size <= 2000; size += 200)
            {
                ...
                cout << "\n__________________" << size << "___________________";
                fout << "\n__________________" << size << "___________________";
