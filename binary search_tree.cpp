...
int BST::t_height(Node1* node) {
    if (node == NULL) {
        ...
        return 1 + max(left_height, right_height); // Возвращаем максимальную высоту поддерева +1
    }
}
...
Node1* BST::delete_node(Node1* node, int num) {
    ...
    if (num < node->key)
        ...
        if (node->left != NULL && node->right != NULL) { // Добавленная проверка
...
void fillRandomArray(int arr[], int size)
{
    ...
    {
        arr[i] = rand() % 50; // Генерируем случайное число от 0 до 500
    }
}
